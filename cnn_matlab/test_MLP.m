...
model = Network();
model = add(model, fcLayer('fc1',784,256,0.01,0.08,0.005,0.9));
model = add(model, Sigmoid('sigmoid1'));
model = add(model, fcLayer('fc2',256,10,0.01,0.03,0.005,0.9));
model = add(model, Sigmoid('sigmoid2'));
%model = add(model, fcLayer('fc3',10,10,0.01,0.01,0.005,0.9));
