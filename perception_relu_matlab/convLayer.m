classdef convLayer
    ...
    methods
        function layer = convLayer(name, kernel_size, num_input, num_output, pad,...
            ...
            layer.num_input = num_input; %the channal
            layer.num_output = num_output;% first time we have 8 filters, second time we have 4 filters..
            layer.pad = pad; %the margine

            layer.learning_rate = learning_rate;
            layer.weight_decay = weight_decay;
            layer.momentum = momentum;

            layer.W = single(random('norm', 0, init_std, kernel_size, kernel_size, num_input, num_output));%if it has multiple channels, do they have the same filter?
        ...
        function layer = backprop(layer, delta)
            ...
            [layer.delta, layer.grad_W, layer.grad_b] = nnconv_bp(layer.input, delta, layer.W, layer.b, layer.pad);
